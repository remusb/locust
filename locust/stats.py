...
"""Default interval for how frequently stats are sent to CloudWatch if this
option is configured."""
CW_STATS_INTERVAL_SEC = 10
...
def stats_cloudwatch():
    """Writes the data for the locust run to CloudWatch."""
    while True:
        write_stat_cw()
        gevent.sleep(CW_STATS_INTERVAL_SEC)

def write_stat_cw():
    # Put custom metrics
    metricData = requests_cw()
...
def requests_cw():
    ...
    for key in six.iterkeys(runners.locust_runner.request_stats):
        s = runners.locust_runner.request_stats[key]
        if s.name is None or s.method is None:
            continue
        ...
        metrics = [{
            'MetricName': 'Num Requests',
            'Dimensions': dimensions,
            'Unit': 'Count',
            'Value': s.num_requests
        }, {
            'MetricName': 'Num Failures',
            'Dimensions': dimensions,
            'Unit': 'Count',
            'Value': s.num_failures
        }, {
            'MetricName': 'Median Response Time',
            'Dimensions': dimensions,
            'Unit': 'Milliseconds',
            'Value': s.median_response_time
        }, {
            'MetricName': 'Avg Response Time',
            'Dimensions': dimensions,
            'Unit': 'Milliseconds',
            'Value': s.avg_response_time
        }, {
            'MetricName': 'Min Response Time',
            'Dimensions': dimensions,
            'Unit': 'Milliseconds',
            'Value': s.min_response_time
        }, {
            'MetricName': 'Max Response Time',
            'Dimensions': dimensions,
            'Unit': 'Milliseconds',
            'Value': s.max_response_time
        }, {
            'MetricName': 'Avg Content Length',
            'Dimensions': dimensions,
            'Unit': 'Bytes',
            'Value': s.avg_content_length
        }, {
            'MetricName': 'RPS',
            'Dimensions': dimensions,
            'Unit': 'Count/Second',
        ...
        if s.num_requests:
            for p in [0.5, 0.66, 0.75, 0.80, 0.90, 0.95, 0.98, 0.99, 1.00]:
                metrics.append({
                    'MetricName': '%d%% Response Time' % int(p*100),
                    'Dimensions': dimensions,
                    'Unit': 'Milliseconds',
                    'Value': s.get_response_time_percentile(p)
                })

    # for key in six.iterkeys(runners.locust_runner.stats.errors):
    #     s = runners.locust_runner.stats.errors[key]
    #     metrics.append({
    #         'MetricName': 'Errors',
    #         'Dimensions': [{
    #             'Name': 'NAME',
